...
if __name__ == "__main__":
    ...
    # enter training mode, drill the existing vocab in randomly formed sentences
        ...
        # systematically create every possible verb-tense-subject combo from the user-specified pool
        concat_str_list = []
        for tense in tense_selection:
            for subject in langSpec['verbConjugations']:
                concat_str_list.append('verbs-' + tense + '-' + subject)

        # print(concat_str_list)

        # each verb in the verb_selection list must have a corresponding concat_str_list, to record which scenarios are still to be tested
        verb_tense_subject_dict = {}
        for verb in verb_selection:
